...
compile_principal_summary_los_data <- function(los_tbl, site, this_measure) {
  summary_los_data <- los_tbl |>
    dplyr::filter(dplyr::if_any("measure", \(x) x == {{ this_measure }})) |>
    dplyr::summarise(
      dplyr::across(c("baseline", "principal"), sum),
      .by = c("pod", "sitetret", "los_group") # also tretspef?
    ) |>
